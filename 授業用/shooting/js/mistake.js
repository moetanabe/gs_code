 ...
 // const ball = {
 //     // src:"images/pizza.png",//画像のURL 
    
 // }
 
 //ボールをえがく
 const ballDraw = function(e){
     console.log(e);
     // const newBall = $("<img>").attr("src",ball.src);
     const newBall = $("<img>").attr("src",ball.img);
     newBall.on("load",function(){
         ball.img = this;
         ball.width=this.width;
         ball.height=this.height;
    
         ctx.drawImage(this, ball.posX, ball.posY);
     }); 
 };
 ...
 const shootBall = function(e){
     ...
     ctx.drawImage(newShootBall.img,newShootBall.posX, newShootBall.posY, newShootBall.width, newShootBall.height);
 ...
 const moveBall=function(){
     for(let i=0;i<ballGroup.length;i++){               
         ...
         ctx.drawImage(ball.img, ball.posX, ball.posY);           
     }
 };
